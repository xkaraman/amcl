...
AMCLDepth::AMCLDepth():m_NumberOfParticles(1000),
...
//	m_MapModel = new OccupancyMap(&m_NH);
	...
	m_MotionModel = new RobotMovementModel(&m_NH,&m_TFBuffer,m_OdomFrameID,m_BaseFrameID);
	m_ObservationModel= new RobotObservationModel(&m_NH,m_MapModel);

	m_ParticleFilter = new libPF::ParticleFilter<RobotState>(m_NumberOfParticles,m_ObservationModel,m_MotionModel);
	...
	m_PoseArrayPub = m_NH.advertise<geometry_msgs::PoseArray>("amcldepth/particlecloud",10);
	m_DepthFilteredPub = m_NH.advertise<PointCloud>("amcldepth/depthfiltered",20);

	// Advertise Services and Initialize Subscribers
	m_GlobalLocService = m_NH.advertiseService("amcldepth/global_localization",&AMCLDepth::globalLocalizationCallback,this);

	m_LaserScanSub = new message_filters::Subscriber<sensor_msgs::LaserScan>(m_NH, "/scan", 100);
	m_LaserScanFilter = new tf2_ros::MessageFilter<sensor_msgs::LaserScan>(*m_LaserScanSub,m_TFBuffer,m_OdomFrameID,100,m_NH);
	m_LaserScanFilter->registerCallback(boost::bind(&AMCLDepth::laserCallback,this,_1));

	m_DepthScanSub = new message_filters::Subscriber<sensor_msgs::PointCloud2>(m_NH, "/camera/depth/points", 100);
	m_DepthScanFilter = new tf2_ros::MessageFilter<sensor_msgs::PointCloud2>(*m_DepthScanSub,m_TFBuffer,m_OdomFrameID,100,m_NH);
	...
	m_InitPoseFilter = new tf2_ros::MessageFilter<geometry_msgs::PoseWithCovarianceStamped>(*m_InitPoseSub,m_TFBuffer,m_MapFrameID,2,m_NH);
	m_InitPoseFilter->registerCallback(boost::bind(&AMCLDepth::initPoseCallback,this,_1));

	// Timer for resending the latest transform
	m_LatestTransformTimer = m_NH.createTimer(ros::Duration(0.1),&AMCLDepth::latestTimerCallback,this);
...
void AMCLDepth::laserCallback(sensor_msgs::LaserScanConstPtr const &msg) {
	...
	if (!m_Initialized) {
		ROS_WARN(
				"Localization not initialized yet, skipping LaserScan input"
	...
	if ( m_ReceivedSensorData && timediff < 0) {
		ROS_WARN("Ignoring received laser data that is %f s older than previous data!",
				timediff);
		return;
	}
	...
	if(!m_FirstRun){
		...
		if(!m_ReceivedSensorData || isAboveMotionThreshold(odomPose)){
			...
			if (!m_MotionModel->lookupTargetToBaseTransform(pcFiltered.header.frame_id,t, sensorToBase) ){
				return;
			}
			...
			m_ObservationModel->setObservedMeasurements(pcFiltered,laserRanges);
...
void AMCLDepth::prepareLaserPointCloud(
	...
	for (int beamId = 0; beamId < numBeams; beamId += step){
		...
		if (range >= laserMin && range <= m_FilterMaxRange){
			double laserAngle = laser->angle_min + beamId * laser->angle_increment;
...
//			tf2::fromMsg(m_ParticleFilter->getMmseEstimate().getPose(), tmp);
			...
			tmpStamped.header.stamp = m_LastLaserTime;
...
//	sor.filter(*cloud);
//
//	writer.write<pcl::PointXYZ>("downsampled.pcd", *cloud, false);
//
//
//	std::cerr << "Cloud before filtering: " << std::endl;
//	std::cerr << *cloud << std::endl;
//
//	pcl::ProgressiveMorphologicalFilter<pcl::PointXYZ> pmFilter;
//	pmFilter.setInputCloud(cloud);
//	pmFilter.setMaxWindowSize(20);
//	pmFilter.setSlope (1.0f);
//	pmFilter.setInitialDistance (0.5f);
//	pmFilter.setMaxDistance (3.0f);
//	pmFilter.extract (ground->indices);
//
...
bool AMCLDepth::isAboveMotionThreshold(const geometry_msgs::PoseStamped &odomPose) const{
	...
	bool isAbove = odomTransform.getOrigin().length() >= m_ObservationThresholdTrans ||
